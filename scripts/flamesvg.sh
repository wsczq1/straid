...
if [ $1 = 'record' ]; then
    ...
    if [ $2 ]; then
        process=$2
        echo "process name is ${process}"
        processid=$(ps -ef | grep ${process} | grep -v grep | awk '{print $2}')

        rm -rf perf.*
        sudo perf record -F 9999 --call-graph dwarf -p ${processid} -- sleep ${record_time}
        # echo_and_run echo "sudo perf record -F 9999 --call-graph dwarf -p ${processid} -- sleep ${record_time}" | bash -
        exit 0
    fi

    rm -rf perf.*
    echo_and_run echo "sudo perf record -F 9999 --call-graph dwarf -a -- sleep ${record_time}" | bash -
    exit 0
fi
...
if [ $1 = 'record' ]; then
    if [ $2 ]; then
        ...
        rm -rf perf.*
        echo_and_run echo "sudo perf record --call-graph dwarf -p ${processid}" | bash -
        exit 0
    fi
